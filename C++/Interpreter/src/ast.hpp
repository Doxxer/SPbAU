...
public:
    If(ExpressionPtr const &condition, ExpressionPtrs const &content)
        : Expressions(content), condition_(condition)
    {
    }
...
public:
    While(ExpressionPtr const &condition, ExpressionPtrs const &content)
        : Expressions(content), condition_(condition)
    {
    }
...
public:
    FunctionDefinition(std::string const &name,
                       std::vector<std::string> params,
                       ExpressionPtrs const &content)
        : Expressions(content), name_(name), params_(params)
    {
    }
...
public:
    BinaryExpression(const char operation,
                     ExpressionPtr left,
                     ExpressionPtr right,
                     size_t lineNumber)
...
public:
    Condition(std::string const &operation,
              ExpressionPtr left,
              ExpressionPtr right,
              size_t lineNumber)
        : Expression(lineNumber), operation_(operation), left_(left), right_(right)
...
public:
    VariableDefinition(std::string const &name, ExpressionPtr const &expression, size_t line)
        : Expression(line), name_(name), expression_(expression)
    {
    }
...
public:
    Call(std::string const &name, ExpressionPtrs const &params, size_t line)
        : Expression(line), name_(name), params_(params)
    {
    }
